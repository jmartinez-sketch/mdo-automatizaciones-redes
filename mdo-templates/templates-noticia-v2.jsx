...
// ── po-13e · Noticia portrait en navy (misma estructura que po-13d) ──
// Mismos slots que po-13d. Existe para que dos noticias de la misma semana
// se distingan de un vistazo en la grilla del perfil: una en papel, otra en
// navy. No cambia la composición — solo el modo de color.
function PoNoticiaV2Navy(props) {
  const p = Object.assign({
    categoria: '[CATEGORIA]', titular: '[TITULAR]', bajada: '[BAJADA]',
    cierre: '[CIERRE]', fuente: '[FUENTE]', fecha: '[FECHA]', handle: '[HANDLE]',
  }, props);
  ...
  return (
    <div className="tpl navy" style={{ padding: 52, display: 'flex', flexDirection: 'column',
      ...
      <IsoWatermark mode="light" size={280} opacity={0.07}
        style={{ position: 'absolute', right: -70, bottom: 84 }} />

      <TplHeader chip="Noticia" mode="light" size={26} />
      <div className="hair" style={{ marginTop: 20 }}></div>

      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center',
        position: 'relative', zIndex: 1 }}>

        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 18 }}>
          <span style={{ display: 'inline-block', width: 24, height: 1, background: 'var(--blue-lt)' }}></span>
          <span className="eyebrow">{p.categoria}</span>
        </div>
        ...
        <div style={{ marginTop: 26, paddingLeft: 18, borderLeft: '2px solid var(--blue)' }}>
          <div className="display-serif" style={{ fontSize: cSize, color: 'var(--blue-lt)',
            lineHeight: 1.18 }}>
            <em>{p.cierre}</em>
          </div>
        </div>
...
const EXAMPLES_NOTICIA_V2 = {
  ...
  PoNoticiaV2Navy: {
    ...
    titular: 'ARCA unifica los regímenes de percepción de IVA',
    bajada: 'Los agentes de percepción pasan a informar bajo un único régimen, con una sola presentación mensual.',
    cierre: 'Menos trámites, pero conviene revisar cómo queda cargado el padrón.',
...
Object.assign(window, { PoNoticiaV2, PoNoticiaV2Navy, SqNoticiaV2, EXAMPLES_NOTICIA_V2 });
